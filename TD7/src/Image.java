...
public class Image {
	...
	// **PERSONNAL**
	public Image(PPM img) {  // Create an Image object from PPM object
		this.width  = img.width;
		this.height = img.height;
		raster = new int[width*height];
		for (int y=0;y<height;y++) {
			for (int x=0;x<width;x++) {
				raster[y * width + x]=0xFF << 24;
				setRed  (x, y, img.r[y][x]);
				setGreen(x, y, img.g[y][x]);
				setBlue (x, y, img.b[y][x]);
			}
		}
	}
	// *************
	...
	public void writeBinaryPPM(String filename) { // Save in ppm file
		String line;
		StringTokenizer st;
		int i;
		try {
	...
	public static void main(String[] args) {
		Image example=new Image("forfloodfilling.png");
		//example.toGray();
		new ImageViewer(example);
		example=new Image(new PPM("forfloodfilling.ppm"));
		//example.toGray();
		new ImageViewer(example);
	}
