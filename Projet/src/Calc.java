...
public class Calc {
	...
	public static Matrix rodrigues(Matrix Rot) {
		...
		if (r.getColumnDimension() == 1 || r.getRowDimension() == 1) {
			...
			res.set(0,1, r.get(0,2) - r.get(2,0));
			res.set(0,2, r.get(1,0) - r.get(0,1));
	...
	public static Matrix[] solvePose(Stack<Pt2> pts2D, Stack<Pt3> pts3D, Matrix A, Matrix dist_coeffs) {    // Return {R,t}. DistCoeff = {{k1,k2,k3,p1,p2}} || {{k1,k2,p1,p2}} || {{k1,k2,k3,p1,p2}}^T || {{k1,k2,p1,p2}}^T || null.
		Matrix R; Pt3 t;
		int nPts = pts3D.size();
